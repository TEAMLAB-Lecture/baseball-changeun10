...
def is_duplicated_number(three_digit):
    digit_list=list(str(three_digit))
    digit_set=set(digit_list)
    if len(digit_set)==len(digit_list):
        return False
    else:
        return True
    # '''
...
def is_validated_number(user_input_number):
    if user_input_number.isdigit()==True:
        if is_between_100_and_999(user_input_number) == True and is_duplicated_number(user_input_number)==False:
            return True
    return False
    # '''
    # Input:
...
def get_not_duplicated_three_digit_number():
    while True:
        a=get_random_number()
        if is_duplicated_number(a)==False:
            break
    return a
    # '''
    # Input:
